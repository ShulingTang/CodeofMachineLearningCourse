...
def plotFig(w, group):
    ...
    ax.scatter(group[0, :8].tolist(), group[1,:8].tolist(), color='red')
    ax.scatter(group[0, 8:].tolist(), group[1,8:].tolist(), color='green')

    k = w[0][0] / w[1][0]
    xx = (np.arange(0, 30, 0.1)).reshape(1, 300)
    yy = k * xx - 10
...
if __name__ == "__main__":
    ...
    k, w = LDA(x1, x2)
    print("w:", w)
    temp = np.hstack((x1.T, x2.T))
    plotFig(w, np.hstack((x1.T, x2.T)))